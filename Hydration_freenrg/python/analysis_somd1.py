...
for rep in list_of_reps:
    if not os.path.isdir(os.path.join(rep, "solvated_somd1")):
        out.write(
            rep + " failed due to lack of solv directory - likely that Min/Eq failed \n"
        )
        continue
    if not os.path.isdir(os.path.join(rep, "vacuum_somd1")):
        out.write(
    ...
    try:
        pmf_solv, overlap_solv = BSS.FreeEnergy.Relative.analyse(
            os.path.join("./", rep, "solvated_somd1")
        )
        ...
        continue
    print(os.path.join("./", rep, "solvated_somd1"))
    files = [
        os.path.join("./", rep, "solvated_somd1", "freenrg-LJCOR-lam-0.000.dat"),
        os.path.join("./", rep, "solvated_somd1", "freenrg-LJCOR-lam-1.000.dat"),
    ]
    print(os.path.join("./", rep, "solvated_somd1", "freenrg-LJCOR-lam-0.000.dat"))
    corrs = []
    ...
    out.write(
        "\n Overall Hydration Free Energy for this run = "
        + str(free_en[0].value() + lj_correction)
    )
    s_r += 1
